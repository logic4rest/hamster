...
def operate_gripper_and_transport(hamster, cap, mapped_category: str, conf: float, stats: dict):
    ...
    while time.time() - prompt_start < 10.0:
        ...
        if ret and frame is not None:
            ...
            frame = draw_hud_and_bbox(frame, f"★ 확정: {mapped_category}", conf, REQUIRED_FRAMES, REQUIRED_FRAMES, stats, prompt_msg)
            cv2.imshow("Waste Sorting Hamster", frame)

        key = cv2.waitKey(20) & 0xFF
        if keyboard.is_pressed("enter") or key in [13, ord('\r')]:
            confirmed = True
            break
        elif keyboard.is_pressed("space") or keyboard.is_pressed("esc") or key in [27, 32]:
            confirmed = False
            break
